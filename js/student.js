...
const getStudentElement = (student) => `
    ...
    <div class="student-subcontainer-all">
        ...
        <div class="student-subcontainer-3">
            ...
            <div class="student-age">Umur: <span class="student-age-dynamic">${calcAge(
              student.birthday
            )}</span> tahun</div>
...
const getSearchedStudents = async (value) => {
  try {
    const rawData = await fetch(
      `../../rest/search-student.php?key=name&value=${value}`
    );
    const data = await rawData.json();
    return data;
  } catch (err) {
...
const displayStudents = async () => {
  ...
  const studentElementString = students
    .map((student) => {
      return `
